...
function calculate() {
  ...
  const err = validateAffordabilityInputs(annualIncome, monthlyDebts, downPayment, annualRate, loanYears);
  ...
  setSubtitle("maxHomePriceResult", `Limited by ${r.limitingFactor === "front-end" ? "housing budget" : "total debt load"}`);
  ...
  lastSummary = `Mortgage Affordability Estimate\n\n` +
