...
function calculate() {
  ...
  const err = validateSpeedInput(input, lang);
  if (err) {
    showError(err);
    return;
  }
  ...
  setSubtitle("convertedResult", getSpeedLabel(input.toUnit, lang));
...
function handleCopy() {
  ...
  copyToClipboard(copySpeedSummary(lastInput, result, lang));
}
